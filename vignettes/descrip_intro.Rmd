---
title: "Desriptive statistics in rigr"
...
In the output, we can see that overall descriptive statistics, as well as descriptive statistics for each strata (smoke = 1, smoke = 2) are returned in the table.

# Subset

Now suppose we only want descriptive statistics for the height and fev variables for individuals over the age of 10. We first create an indicator variable for age > 10 outside of the `descrip()` function, and then include this variable in the subset parameter.
...
From the output, we can see that 96.92% of the indivuals in this dataset with smoking status 1 had an fev greater than 2, and 71.99% of the individuals in this dataset with smoking status 2 had an fev greater than 2.
