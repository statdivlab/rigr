...
The result the same as that provided by `ttest(mri$weight, null.hypoth = 155)`. 
...
```{r}
proptest(as.numeric(mri$ldl > 128), null.hypoth = 0.5, exact = FALSE)
proptest(as.numeric(mri$ldl > 128), null.hypoth = 0.5, exact = TRUE)
```

Note that we are creating our binary data within the `proptest` call. Using the `exact` argument allows us to choose what kind of test we run. In this case, the results are quite similar. 
