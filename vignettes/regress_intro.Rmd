...
Note that again we have two tables of output, denoted by `Raw Model` and `Transformed Model`, with `Transformed Model` displaying expotentiated estimated coefficients. 
...
Suppose we're interested in testing whether or not a certain linear combination of coefficients in our regression model is equal to a given null hypothesis. We can use the `lincom` function to perform this test. Consider the regression model below:

```{r}
mod_rigr <- regress("mean", atrophy ~ age + sex + height + weight, data = mri)
```

If we want to test the null hypothesis that the coefficient created by `height + weight` is equal to zero, we can specify this test using `lincom` as follows.

```{r}
# The elements in mod_combo correspond to Intercept, age, sexMale, height, and weight
mod_combo <- c(0, 0, 0, 1, 1)
lincom(mod_rigr, mod_combo)
```

Suppose we instead want to test the null hypothesis that the coefficient created by `2 * age + sexMale` is equal to 50. We specify this by,

```{r}
mod_combo <- c(0, 2, 1, 0, 0)
lincom(mod_rigr, mod_combo, hyp = 50)
```
