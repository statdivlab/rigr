...
# Linear Regression
As a first example, we run a linear regression analysis of atrophy on age and male, from the `mri` data. This dataset is included in the `rigr` package; see its documentation by running `?mri`.
...
```{r}
regress("mean", atrophy ~ age + I(sex), data = mri)
```
...
```{r}
regress("odds", diabetes ~ I(sex), data = mri)
```
...
Proportional hazards regression and generalized estimating equations (GEE) are no longer supported.

# Re-parameterizations of a Variable

There are three special functions in `rigr` which allow us to re-parameterize variables:
...
TODO: fix! This is biological racism. Can use any other variable instead, e.g. stroke. 

```{r}
regress("odds", diabetes ~ dummy(stroke), data = mri)
```
...
```{r}
# TODO
# regress("mean", atrophy ~ U(ma = ~sex*age) + U(mr = ~sex*dummy(race)), data = mri)
```
